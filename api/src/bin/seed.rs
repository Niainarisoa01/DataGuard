...
async fn main() -> anyhow::Result<()> {
    ...
    let account = sqlx::query!("SELECT id FROM accounts WHERE email = $1", "test@dataguard.local")
        .fetch_one(&pool)
        .await?;
    ...
    println!("========================================");
    println!(
        "➡️  Utilisez cette clé dans le header 'X-Api-Key' de vos requêtes Postman/Bruno/cURL."
    );
