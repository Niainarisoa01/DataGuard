...
pub async fn validate_csv(
    ...
    let compiled_schema = JSONSchema::compile(&schema_json).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    ...
    tokio::spawn(async move {
        ...
        let mut iter = csv_reader.deserialize::<std::collections::HashMap<String, String>>();
                ...
                Ok(hashmap) => {
                    ...
                    if !compiled_schema.is_valid(&row) {
                                if let Err(errs) = compiled_schema.validate(&row) {
                                    let errors_str: Vec<String> = errs.map(|e| e.to_string()).collect();
                                    let errors_joined = errors_str.join("; ");
                                    
                                    let raw_json = serde_json::to_string(&row).unwrap_or_default().replace("\"", "\"\"");
                        ...
                        Err(e) => {
                            ...
                            let line = format!("{},\"Parse error: {}\",\"\"\n", row_num, parse_err.to_string().replace("\"", "\"\""));
                            let _ = tx.send(Ok(Bytes::from(line))).await;
                        }
                    }
                    row_num += 1;
